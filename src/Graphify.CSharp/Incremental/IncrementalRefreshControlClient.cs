...
internal sealed class IncrementalRefreshControlClient
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var pipeName = IncrementalRefreshControlChannel.ForRequest(request);
        ...
        while (true)
        {
            ...
            if (response is not null)
            {
                if (!response.Success)
                {
                    throw new InvalidOperationException(response.Message ?? "The watcher rejected the refresh request.");
                }
        ...
        CancellationToken cancellationToken)
    {
        ...
        await writer.WriteLineAsync(JsonSerializer.Serialize(new { command }, JsonOptions))
